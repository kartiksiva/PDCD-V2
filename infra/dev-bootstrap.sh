...
PROVISION_LEGACY_APP_SERVICE="${PROVISION_LEGACY_APP_SERVICE:-false}"
...
maybe_provision_legacy_app_service() {
  if [[ "${PROVISION_LEGACY_APP_SERVICE}" == "true" ]]; then
    info "PROVISION_LEGACY_APP_SERVICE=true; provisioning legacy App Service rollback path"
    ensure_app_service
    return
  fi

  info "Skipping legacy App Service provisioning (set PROVISION_LEGACY_APP_SERVICE=true to create rollback web apps)"
}
...
if [[ "${PROVISION_LEGACY_APP_SERVICE}" == "true" ]]; then
  az resource show --ids "/subscriptions/$SUBSCRIPTION_ID/resourceGroups/$RESOURCE_GROUP/providers/Microsoft.Web/sites/$WEBAPP_NAME" --query "defaultHostName" -o tsv | sed 's|^|Legacy App Service API host: https://|' | sed 's|$|/|' | tr -d '\n'
  echo
else
  echo "Legacy App Service rollback path not provisioned by default."
fi
